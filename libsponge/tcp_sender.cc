...
void TCPSender::fill_window() {
    ...
    while(!_stream.input_ended()&&_rcv_window_size>0&&_stream.buffer_size()>0){
        ...
        // std::cout<<"seg_payload_len: "<<seg_payload_len<<"\n";
        // std::cout<<"endinput: "<<_stream.input_ended()<<"\n";
...
bool TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if(ab_ack>=_next_seqno) return false;
        ...
        if(!_segments_out.empty()) _timer=_time_passed;//restart timer;
    }
    return true;
}

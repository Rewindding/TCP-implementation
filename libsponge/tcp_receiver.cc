...
bool TCPReceiver::segment_received(const TCPSegment &seg) {
    ...
    auto seqno_end=seqno_start+max(seg.length_in_sequence_space()-1,static_case<size_t>(0));
    auto window_start=unwrap(_ack,_isn,_checkpoint); 
    //window size==0的时候需要发送方持续发送数据，直到窗口size不等于0
    auto window_end=window_start+max(window_size()-1,static_case<size_t>(0));//if window size==0?
